...
namespace Creative8\CartRecovery\Email;

use Creative8\CartRecovery\CartRecovery;
use Creative8\CartRecovery\CronHandler;
 ...
 * AbandonedCart class
 */
class AbandonedCart extends \WC_Email {
	...
	protected function get_dummy_cart_data() {
		...
		$dummy_cart->cart_contents = serialize(
			array(
				array(
					'product_id'    => 0,
					'variation_id'  => 0,
	...
	public function trigger( $cart ) {
		...
		if ( is_object( $cart ) ) {
			...
			$this->recovery_url    = CartRecovery::get_recovery_url( $cart->recovery_token );
			$this->unsubscribe_url = CartRecovery::get_unsubscribe_url( $cart->recovery_token );
		...
		if ( $this->is_enabled() && $this->get_recipient() ) {
			...
			if ( $result ) {
				// Mark email as sent
				CronHandler::mark_email_sent( $cart->id );
